...
openai.api_version = (
    AZURE_OPENAI_API_VERSION  # Ensure it's compatible with your deployment
)
            ...
            messages=[
                {
                    "role": "system",
                    "content": f"""
        You are a podcast conversation generator that produces **valid SSML** output in {language}. Your output must be **well-formed XML** following the SSML standard. Follow these rules **strictly**:

        1. **XML Structure:**
        - Enclose the entire output in a single `<speak>` tag.
        - The `<speak>` tag must include these attributes:
        - `version="1.0"`
        - `xmlns="http://www.w3.org/2001/10/synthesis"`
        - `xml:lang="{language}"`
        - **Do not include any content before `<speak>` or after `</speak>`.**

        2. **Dialogue Format:**
        - Create a dialogue between two speakers using the placeholders `{speaker1}` and `{speaker2}`.
        - Each dialogue turn should be encapsulated within a `<voice>` tag with the corresponding `name` attribute.
        - Inside each `<voice>`, wrap the spoken text in a `<prosody>` tag with the attribute `rate="medium"`. **Do not use other values like "fast".**
        - **Ensure there are more than 20 dialogue interactions** (i.e., alternating turns between the two speakers).

        3. **Content Guidelines:**
        - **Introduction:** Start with a brief introduction to the topic.
        - **Discussion:** Follow with a lively discussion covering key points, details, and examples.
        - **Conclusion:** End with a summarizing conclusion.
        - **Language Consistency:** All text must be in {language}.

        4. **Formatting and Validation:**
        - Use `<emphasis>` tags sparingly to highlight key phrases without compromising XML validity.
        - Verify that all XML tags are properly opened and closed; avoid mismatched, duplicate, or misplaced tags and characters.
        - **Output only valid SSML.** If any syntax issues arise, fix them before finalizing your output.
        - **Not add '''xml at the beginning of the SSML content or ``` at the end.

        **Example Format:**
            <speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="{language}">
                <voice name="{speaker1}">
                    <prosody rate="medium">Welcome to our podcast! Today we're discussing cutting-edge technology.</prosody>
                </voice>
                <voice name="{speaker2}">
                    <prosody rate="medium">Absolutely, we'll explore various aspects and real-world applications.</prosody>
                </voice>
            <!-- Continue with additional dialogue turns (At least 20 interactions, but it can extend to whatever you believe makes sense) -->
        </speak>
        """,
                },
                {
...
def split_ssml(ssml_content, max_voice_elements=50):
    ...
    """
    print(
        f"SSML content before parsing: {ssml_content}"
    )  # Debug print to see SSML content before parsing
    try:
    ...
    if not voices:
        print(
            "No <voice> elements found in the SSML. Make sure the SSML content has properly formatted <voice> tags."
        )
        return []

    chunks = []
    current_chunk = ET.Element(
        "speak",
        {
            "version": "1.0",
            "xmlns": "http://www.w3.org/2001/10/synthesis",
...
def generate_audio():
    ...
    for idx, ssml_chunk in enumerate(ssml_chunks):
        ...
        if not generate_speech(ssml_chunk, file_path, speech_config):
            return (
                jsonify({"error": f"Error generating the audio for chunk {idx+1}."}),
                500,
            )
        ...
        try:
            ...
            return (
                jsonify({"error": f"Error uploading audio to Azure Blob Storage: {e}"}),
                500,
            )
    ...
    if len(audio_files) > 1:
        ...
        try:
            ...
            combined_blob_url = upload_to_blob_storage(
                combined_file_name, combined_file_path
            )
        except Exception as e:
            return (
                jsonify(
                    {
                        "error": f"Error uploading combined audio to Azure Blob Storage: {e}"
                    }
                ),
                500,
            )
